		...
		<v-row no-gutters>
			...
			>
				...
				<div align="center" class="mb-5">
					Articles, tutoriels, documentations techniques, revues, rapport, recherches, cours..
					<br>
					L'ensemble de ma bibliographie est ici. 
				</div>
				<articles-by-year v-for="(item, i) in articles_by_year" :key="i" :year="item.year" :articles="item.articles_list" />
			</v-col>
		</v-row>
	</v-container>
</template>
...
export default {
	...
	async mounted() {
		...
		this.articles.forEach(article => {
			...
			if (this.articles_by_year.some(elem => elem.year === article_year)) {
				const index = this.articles_by_year.findIndex(elem => elem.year === article_year)
			...
			// then I can push the article into the new empty articles_list I just created
				...
				this.articles_by_year[
					this.articles_by_year.push({year: article_year, articles_list: []})-1
